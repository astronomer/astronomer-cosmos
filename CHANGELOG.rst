...
* ``emit_datasets`` is now a template field on the dbt operators, so emission can be decided per DAG run: ``operator_args={"emit_datasets": "{{ dag_run.run_type != 'backfill' }}"}`` stops backfills from triggering downstream asset-scheduled DAGs. Only ``operator_args`` is templatable, and it now takes precedence over ``RenderConfig.emit_datasets``, which stays a plain boolean read at DAG parse time. Previously an ``operator_args`` value was silently discarded, so ``operator_args={"emit_datasets": False}`` starts taking effect.
