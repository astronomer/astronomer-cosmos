...
class DbtTestWarningHandlerGcpGke(KubernetesPodOperatorCallback):  # type: ignore[misc]
    ...
    def on_pod_completion(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        """
        if not self.context:
            self.operator.log.warning("No context provided to the DbtTestWarningHandlerGcpGke.")
            return

        task = self.context["task_instance"].task
        if not (isinstance(task, DbtTestGcpGkeOperator) or isinstance(task, DbtSourceGcpGkeOperator)):
        ...
        if isinstance(task, DbtTestGcpGkeOperator):
        ...
        elif isinstance(task, DbtSourceGcpGkeOperator):
...
class DbtWarningGcpGkeOperator(DbtGcpGkeBaseOperator, ABC):
    ...
    def __init__(self, *args: Any, on_warning_callback: Callable[..., Any] | None = None, **kwargs: Any) -> None:
        ...
        if on_warning_callback:
            self.warning_handler = DbtTestWarningHandlerGcpGke(on_warning_callback, operator=self)
