...
class DbtConsumerWatcherKubernetesSensor(BaseConsumerSensor, DbtRunKubernetesOperator):
    ...
    (OpenLineage, OpenMetadata, etc.). Mirrors ``DbtConsumerWatcherSensor`` (the
    SUBPROCESS equivalent), but delegates to ``register_dataset_on_task`` because this
    sensor doesn't inherit from the local operator.
    """
    ...
    def _emit_datasets(self, context: Context) -> None:
        ...
        else:  # pragma: no cover - WATCHER_KUBERNETES requires a cncf-kubernetes provider only present on Airflow 3 CI jobs
