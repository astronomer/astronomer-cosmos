...
class DbtArgs(TypedDict, total=False):
    """
    Maintain all available arguments here so that we can help end users with available kwargs.
    """

    project_dir: Path  # Not available for end users, generated by Cosmos
    base_cmd: str | list[str]
    select: str
    exclude: str
    selector: str
    vars: dict[str, str]
    models: str  # Not available for end users, generated by Cosmos
    ...
    output_encoding: str
    skip_exit_code: int
    cancel_query_on_kill: bool
    dbt_executable_path: str
    full_refresh: bool  # Available on seed only
    macro_name: str  # Available on run operation only
    ...
    def run_command(self, cmd, env):
        ...
        if project_dir.exists():
            if project_dir.is_dir():
                ...
                msg = f"The {project_dir=} must be a directory"
        else:
            msg = f"Cannot find {project_dir=}"
