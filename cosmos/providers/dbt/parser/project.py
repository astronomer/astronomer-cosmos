        ...
        for jinja_call_node in ast.find_all(Call):
            if hasattr(jinja_call_node.node, "name"):
                ...
                if jinja_call_node.node.name in dependency_node_names:
                    # TODO new bug? source has 2 arguments... the model definition comes from sources in config
...
class DbtProject:
    ...
    def __post_init__(self) -> None:
        """
        Parses the dbt models, config yml files and then finally dbt project yml file.
        We must go in order of most specific to the least specific so:
            1. Config in the SQL models.
            2. Config in the schema.yml files, bottom up in the directory structure.
            3. Config in the project.yml file, bottom up in the directory structure.
            4. If nothing is found for the materialization strategy then view is added as per dbt behaviours.
        """
        ...
        # schema.yml files, not project.yml
    ...
    def _handle_project_config_recursive(
        ...
        if config_selectors:
            ...
            for parsed_model_name, parsed_model in self.models.items():
                if parsed_model.path.is_relative_to(project_path):
