...
- ``seed_rendering_behavior``: (new in v1.13) Determines how seed nodes are rendered (ALWAYS, NONE, WHEN_SEED_CHANGES). Defaults to "NONE" (since Cosmos 1.13). Test behavior is controlled separately via ``test_behavior``. See `Seed Nodes Rendering <./seed-nodes-rendering.html>`_ for more information.
