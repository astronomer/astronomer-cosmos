...
- ``env_vars``: (Deprecated since Cosmos 1.3 use ``ProjectConfig.env_vars`` instead) A dictionary of environment variables for rendering. Only supported when using ``load_method=LoadMode.DBT_LS``.
