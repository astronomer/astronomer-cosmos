...
The recommended pattern is to build the dbt artifacts **once** at
...
- ``partial_parse.msgpack`` — Cosmos automatically picks this up from the
  ...
  execution. See :ref:`partial-parsing`.
...
- **Tags and selectors.** When the folder layout doesn't match ownership
  ...
  cleanly. See :ref:`selecting-excluding`.
...
- **Ownership and on-call.** If different teams own different parts of
  ...
  :ref:`task callbacks <callbacks>` can then map
...
- Upload artifacts to object storage (S3, GCS, Azure WASB) — Cosmos ships
...
See :ref:`callbacks` for the full callback API,
