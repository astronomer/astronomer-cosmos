...
10. **Dataset events not emitted**: Dataset events are not currently emitted after dbt models complete when using ``ExecutionMode.AIRFLOW_ASYNC``. This means downstream DAGs scheduled with ``Dataset`` or ``DatasetAlias`` will not trigger automatically. This behavior is present in ``ExecutionMode.LOCAL`` but currently missing in async mode. A fix is being tracked in issue `#2141 <https://github.com/astronomer/astronomer-cosmos/issues/2141>`_.
