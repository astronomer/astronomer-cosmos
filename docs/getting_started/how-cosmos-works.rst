...
- **Parse your dbt project**: Cosmos parses your dbt project, and translates it into an Airflow Dag. This process uses the :ref:`ProjectConfig <project-config>` and :ref:`RenderConfig <render-config>` to customize specific behavior, allowing you to optimize how your dbt project is represented as a Dag.

- **Execute the dbt commands**: Cosmos then executes the Dag, using the execution options in your :ref:`ExecutionConfig <execution-config>` and :ref:`ProjectConfig <project-config>` to run dbt commands with the appropriate dbt adapter, finally resulting in your dbt SQL running in your data warehouse. Cosmos uses a connection defined in the :ref:`ProfileConfig <profile-config>` to execute your SQL in your data warehouse.
...
Even though Cosmos is highly extensible, and you have many advanced customization options, you can run a demo with the :ref:`Astro CLI <astro-cli-quickstart>` in just a few minutes. This demo introduces you to the key elements required for Cosmos to parse dbt projects and run Dags.
