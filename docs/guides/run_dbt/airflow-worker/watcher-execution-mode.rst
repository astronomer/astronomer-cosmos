...
``ExecutionMode.WATCHER`` requires **dbt-core 1.5 or newer**, which Cosmos's
own :doc:`minimum supported dbt Core version </policy/compatibility-policy>`
(1.8) already satisfies. The watcher relies on ``--log-format json`` to
consume structured per-model events from dbt, with the flag placed after the
dbt subcommand (e.g. ``dbt build --log-format json …``). dbt-core 1.5
introduced finer-grained log CLI parameters (see the
