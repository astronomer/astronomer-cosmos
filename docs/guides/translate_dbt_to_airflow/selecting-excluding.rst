...
- ``resource_type:<resource>``: include nodes with the resource type ``seed, snapshot, model, test, source, semantic_layer``. For example, ``resource_type:source`` returns only nodes where resource_type == SOURCE. ``semantic_layer`` matches adapter-native semantic layer objects (e.g. Databricks metric views, Snowflake semantic views) — dbt models this as ``config.materialized: metric_view`` (or ``semantic_view``), and Cosmos reclassifies them out of ``model`` accordingly.
