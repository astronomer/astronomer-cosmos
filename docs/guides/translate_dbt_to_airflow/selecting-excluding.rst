...
- ``resource_type:<resource>``: include nodes with the resource type ``seed, snapshot, model, test, source, semantic_layer``. For example, ``resource_type:source`` returns only nodes where resource_type == SOURCE. ``semantic_layer`` matches adapter-native semantic layer objects reclassified by Cosmos at parse time; see :doc:`Managing semantic layer <managing-semantic-layer>` for prerequisites and details.
