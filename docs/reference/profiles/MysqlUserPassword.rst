...
This profile mapping translates Airflow connections with the type ``mysql``
into dbt profiles. To use this profile, import it from ``cosmos.profiles``:

.. code-block:: python

    from cosmos.profiles import MysqlUserPasswordProfileMapping

    profile = MysqlUserPasswordProfileMapping(
