...
| AIRFLOW_ASYNC (BigQuery) | `DbtRunAirflowAsyncBigqueryOperator._register_event` (`cosmos/operators/_asynchronous/bigquery.py:259-270`) builds the URI itself, branching on the Airflow version: `f"bigquery://{gcp_project}/{dataset}/{table_name}"` on Airflow 3 and `f"bigquery://{gcp_project}.{dataset}.{table_name}"` on Airflow 2. It mirrors `construct_dataset_uri`'s delimiter switch without calling it. | No, on both versions, and for three independent reasons: the scheme is a hardcoded `bigquery://` rather than `get_dataset_namespace`'s output; the name is `unique_id.split(".")[-1]`, so a model with a custom `alias` gets the model name instead of the resolved relation; and nothing is `urllib.parse.quote`d. Matching the delimiter convention on each version doesn't make the URI match. |
