...
class TestWatcherTrigger:
    ...
    async def test_get_xcom_val_af3(self):
        ...
        with patch("cosmos.operators._watcher.triggerers.sync_to_async") as mock_sync_to_async:
    ...
    async def test_get_xcom_val_af2(self):
        ...
        with patch("airflow.utils.session.create_session", return_value=mock_session):
            ...
            with patch("cosmos.operators._watcher.triggerers.sync_to_async", side_effect=wrap_sync):
    ...
    async def test_parse_node_status(self, use_event, xcom_val, expected_status):
        ...
        with (
            patch("cosmos.operators._watcher.triggerers._parse_compressed_xcom", return_value=xcom_val),
    ...
    async def test_get_xcom_val_branches(self, airflow_version, expected_val):
        with patch("cosmos.operators._watcher.triggerers.AIRFLOW_VERSION", airflow_version):
            if expected_val == "af2":
    ...
    async def test_run_various_outcomes(self, node_status, producer_state, expected):
        ...
        with (
            ...
            patch(
                "cosmos.operators._watcher.triggerers._parse_compressed_xcom",
    ...
    async def test_get_producer_task_status_airflow2(self):
        ...
        with patch("cosmos.operators._watcher.triggerers.AIRFLOW_VERSION", Version("2.9.0")):
            with patch(
                "cosmos.operators._watcher.triggerers.build_producer_state_fetcher", return_value=fetcher
            ) as mock_builder:
    ...
    async def test_get_producer_task_status_airflow2_missing_ti(self):
        ...
        with patch("cosmos.operators._watcher.triggerers.AIRFLOW_VERSION", Version("2.9.0")):
            with patch("cosmos.operators._watcher.triggerers.build_producer_state_fetcher", return_value=fetcher):
                state = await self.trigger._get_producer_task_status()
    ...
    async def test_get_producer_task_status_airflow3(self):
        ...
        with patch("cosmos.operators._watcher.triggerers.AIRFLOW_VERSION", Version("3.0.0")):
            with patch("cosmos.operators._watcher.triggerers.build_producer_state_fetcher", return_value=fetcher):
                state = await self.trigger._get_producer_task_status()
    ...
    async def test_run_poke_interval_and_debug_log(self, caplog):
        ...
        with (
            ...
            patch("cosmos.operators._watcher.triggerers.WatcherTrigger._parse_node_status", parse_node_status_mock),
