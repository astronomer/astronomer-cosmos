...
@pytest.mark.skipif(
    ...
    reason="We are only testing watcher GCP GKE with Airflow 3.1 and more recent versions of the K8s provider",
)
@pytest.mark.integration
def test_dbt_dag_with_watcher_gcp_gke():
    """
    Create a Cosmos DbtDag with `ExecutionMode.WATCHER_GCP_GKE`.
    ...
    dag_dbt_watcher_gcp_gke = DbtDag(
        dag_id="watcher_gcp_gke_dag",
    ...
    run_dag(dag_dbt_watcher_gcp_gke)

    assert len(dag_dbt_watcher_gcp_gke.task_dict) == 9
    tasks_names = [task.task_id for task in dag_dbt_watcher_gcp_gke.topological_sort()]
    ...
    assert isinstance(
        dag_dbt_watcher_gcp_gke.task_dict["dbt_producer_watcher"],
        DbtProducerWatcherGcpGkeOperator,
    )
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["raw_customers_seed"], DbtSeedWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["raw_orders_seed"], DbtSeedWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["raw_payments_seed"], DbtSeedWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["stg_customers_run"], DbtRunWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["stg_orders_run"], DbtRunWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["stg_payments_run"], DbtRunWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["customers_run"], DbtRunWatcherGcpGkeOperator)
    assert isinstance(dag_dbt_watcher_gcp_gke.task_dict["orders_run"], DbtRunWatcherGcpGkeOperator)
